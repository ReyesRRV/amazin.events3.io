...
let arrayComming = data.events.filter(event => parseInt(event.date) >= parseInt(data.currentDate))
...
///////// ---------------------FUNCIÓN CREAR CARTAS---------------------//////
 ...
 <label class="form-check-label text-light shadow rounded" for="${exactamenteLoMismo}">${exactamenteLoMismo}</label>
  ...
  checkbox.forEach((event) =>
    event.addEventListener("click", clickValue)
  );  
  ...
  function filterinData() {
    ...
    if (text !== "" && arrayCatLength > 0) {
      ...
      filterArray.push(...finalArray.filter((event) => event.name.toLowerCase().includes(text.toLowerCase().trim()) &&  arrayChecked.some((item) => item == event.category)
        )
      );
      ...
      ///CASO 3   BUSCAR
      filterArray.push(
        ...finalArray.filter((event) => event.name.toLowerCase().includes(text.toLowerCase().trim())
        )
      );
    } else {
      ////CASO 4
...
function getData() {
  ...
  <div class="row g-0">
    ...
    <h6 class="card-text">${event.date}</small></h6>
        ...
        <h6 class="card-text" id="keyArray"> ${Object.keys(event)[8]}: ${event.assistance || event.estimate } </small></h6>
        <div class="d-flex justify-content-around">
        <h6 class="card-text">Price: $${event.price} <button onclick="javascript:history.back()">Back</small></h6>
        </div>
        </div>
    </div>
  </div>
